...
const Apply_player = require('../models/apply_for_player')
const auth = require('../middleware/auth')
const router = new express.Router()

router.post('/apply_for_player', auth, async (req, res) => {
    let ts = Math.round(Date.now() / 1000);
    let body = req.body
    const apply = new Apply_player({
    ...
    router.get('/applicants_player', auth, async (req, res) => {
        ...
        try {
           const result = await Apply_player.find({
           // age : req.query.age
